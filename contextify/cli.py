...
def main():
    ...
    export_parser.add_argument("--output",
                               ...
                               default=None,
                               help="输出 JSON 文件名")
    export_parser.add_argument("--exts",
                               nargs="+",
                               default=None,
                               help="要收集的文件扩展名")
    export_parser.add_argument("--exclude-dirs",
                               nargs="+",
                               default=None,
                               help="排除目录正则")
    export_parser.add_argument("--include-dirs",
                               nargs="+",
                               default=None,
                               help="仅包含目录正则")
    ...
    if args.command == "export":
        ...
        context, errors, exclude_dirs_used, include_dirs_used = collect_project_context(
            args.project_dir,
            exts=exts,
        ...
        print(f"\n✅ 已导出上下文: {output_file}")
        print(
            f"📦 导出文件: {context['total_files']} 个，累计 {context['total_bytes']} 字节"
        )
        print(f"\n📂 排除目录正则：")
        for pattern in exclude_dirs_used:
            print(f"  - {pattern}")

        if include_dirs_used:
            print(f"\n📂 仅包含目录正则：")
            for pattern in include_dirs_used:
                print(f"  - {pattern}")
